...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode * split(vector<Point<Dim>>& newPoints, int left, int right, int dimension);
    int partition(vector<Point<Dim>>& newPoints, int left, int right, int pivotIndex, int dimension);
    void swap(vector<Point<Dim>>& newPoints, int left, int right);
    Point<Dim> select(vector<Point<Dim>>& newPoints, int left, int right, int k, int dimension);
    void copy_(KDTreeNode*& curr, KDTreeNode*& other);
    ...
    Point<Dim> findNearestNeighbor_(const Point<Dim>&query, KDTreeNode* root, int dimension) const;
